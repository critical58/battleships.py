...
board1 = [[" " for i in range(10)] for i in range(10)]
...
def initBoard():
    ...
    for i in [(5, "C"), (4, "B"), (3, "D"), (3, "S"), (2, "P")]:
        ...
        while True:
            ...
            try:
                ...
                coordStart = input("Player 1, where would you like your " +
                                   str(i[0])+"-long piece to start E.G (A1)?\n>>> ")
                ...
                # get end coord
                coordEnd = input("\nPlayer 1, where would you like your " +
                                 str(i[0])+"-long piece to end E.G (A1)?\n>>> ")
                # calculate points between start and end
                if int(coordStart[1:])-1 == int(coordEnd[1:])-1:
                    # get minimum coord
                    minCoord = min(ord(coordStart[0].lower()) -
                                   97, ord(coordEnd[0].lower()) - 97)
                    # loop through midpoints
                    for j in range(i[0]):
                        # add to list to deny overlaps
                        tempCoords.append(
                            [int(coordStart[1:])-1, j + minCoord])
                ...
                if (((int(coordStart[1:])-1) + (i[0] - 1) == (int(coordEnd[1:])-1) and (ord(coordStart[0].lower()) - 97) == (ord(coordEnd[0].lower()) - 97)) or ((int(coordStart[1:])-1) - (i[0] - 1) == (int(coordEnd[1:])-1) and (ord(coordStart[0].lower()) - 97) == (ord(coordEnd[0].lower()) - 97)) or ((int(coordStart[1:])-1) == (int(coordEnd[1:])-1) and (ord(coordStart[0].lower()) - 97) + (i[0] - 1) == (ord(coordEnd[0].lower()) - 97)) or ((int(coordStart[1:])-1) == (int(coordEnd[1:])-1) and (ord(coordStart[0].lower()) - 97) - (i[0] - 1) == (ord(coordEnd[0].lower()) - 97))) and any(coord in p2coords for coord in tempCoords) == False:
                    ...
                    # check if ship is vertical
                        ...
                        # get minimum coord
                        minCoord = min(
                            int(coordStart[1:])-1, int(coordEnd[1:])-1)
                        # loop through midpoints
                        for j in range(i[0]):
                            p1coords.append(
                                [j + minCoord, ord(coordStart[0].lower()) - 97])
                            ...
                            board1[j +
                                   minCoord][ord(coordStart[0].lower()) - 97] = i[1]
    ...
    for i in range(len(board1)):
        print("|", end="")
        for j in range(len(board1[j])):
            print(str(board1[i][j]) + "|", end="")
        print("")
    print("")
    input("Press ENTER for Player 2\n")
  ...
  # PLAYER 2
    ...
    for i in [(5, "C"), (4, "B"), (3, "D"), (3, "S"), (2, "P")]:
        ...
        while True:
            ...
            try:
                # ask for start coordinate
                coordStart = input("Player 2, where would you like your " +
                                   str(i[0])+"-long piece to start E.G (A1)?\n>>> ")
                # print(int(coordStart[1:])-1) # num section of coords a「3」
                # print(ord(coordStart[0].lower()) - 97) # letter section of coords「a」3
                # mCoordStart = board1[int(coordStart[1:])-1][ord(coordStart[0].lower()) - 97], for copy and paste
                # get end coord
                coordEnd = input("\nPlayer 2, where would you like your " +
                                 str(i[0])+"-long piece to end E.G (A1)?\n>>> ")
                # calculate points between start and end
                if int(coordStart[1:])-1 == int(coordEnd[1:])-1:
                    # get minimum coord
                ...
                if (((int(coordStart[1:])-1) + (i[0] - 1) == (int(coordEnd[1:])-1) and (ord(coordStart[0].lower()) - 97) == (ord(coordEnd[0].lower()) - 97)) or ((int(coordStart[1:])-1) - (i[0] - 1) == (int(coordEnd[1:])-1) and (ord(coordStart[0].lower()) - 97) == (ord(coordEnd[0].lower()) - 97)) or ((int(coordStart[1:])-1) == (int(coordEnd[1:])-1) and (ord(coordStart[0].lower()) - 97) + (i[0] - 1) == (ord(coordEnd[0].lower()) - 97)) or ((int(coordStart[1:])-1) == (int(coordEnd[1:])-1) and (ord(coordStart[0].lower()) - 97) - (i[0] - 1) == (ord(coordEnd[0].lower()) - 97))) and any(coord in p2coords for coord in tempCoords) == False:
                    ...
                    if int(coordStart[1:])-1 == int(coordEnd[1:])-1:
                        ...
                        for j in range(i[0]):
                            ...
                            p2coords.append(
                                [int(coordStart[1:])-1, j + minCoord])
                            # assign values to ship type
                            board2[int(coordStart[1:])-1][j + minCoord] = i[1]
                    # check if ship is vertical
                    else:
                        # get minimum coord
                        minCoord = min(
                    ...
                    for j in range(len(board2)):
                        print("|", end="")
                        for k in range(len(board2[j])):
                            print(str(board2[j][k]) + "|", end="")
                        print("")
                    print("")
                    # go to next iteration if input is allowed
                    break
    ...
    for i in range(len(board2)):
        print("|", end="")
        for j in range(len(board2[j])):
            print(str(board2[i][j]) + "|", end="")
        print("")
    print("")
